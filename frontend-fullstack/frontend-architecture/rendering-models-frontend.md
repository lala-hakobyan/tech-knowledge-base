...
**Server-Side Rendering (SSR)** means that the server generates the HTML for the page and sends it to the browser to render upon receiving a page request from the browser.
...
- With server-side rendering, your server runs the frontend technology to produce an HTML response for every page request, which may **increase server hosting costs**.
...
- Pre-rendering requires that all **information needed to render a page is available at build time**. This means pre-rendered pages cannot include user-specific data. It is primarily useful for **pages that are the same for all users**.
...
- **Server-Side Rendering (SSR)**: HTML is rendered per page request on the server.
...
| **Server-Side (SSR)**             | - Fast initial load<br> - Great SEO<br> - Server-side data fetching                       | - Higher server cost<br> - No browser-only libs<br> - Angular runs on each request          | The server renders HTML and sends to the client for display upon recieving page request from browser.                  | **SEO-critical sites, or content-heavy websites where fast initial load time is a priority (rendering user-specific data immediately to avoid loading spinners).**                    |
| **Prerendering (SSG)**            | - Fastest page load<br> - Great SEO<br> - CDN caching<br> - Zero server load per request  | - Build-time data only<br> - Slower build time<br> - Not for dynamic or user-specific pages | The server generates static HTML on build-time, the client uses JavaScript to update.                                  | **Websites that require fast initial load times and static content (marketing pages, blogs, docs) - content that doesn't depend on individual user data for its initial rendering.**  |
